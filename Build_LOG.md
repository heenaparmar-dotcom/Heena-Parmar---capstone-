...
## Design World MVP v2 — full rebuild: login, real auto-apply agent, deployment

### Entry

- **Date:** 19–25 September 2026
- **Time spent:** Not directly measured (multiple sessions)
- **Approx tokens used:** Not directly measured
- **What shipped:**
  - **Full architecture rebuild on branch `assessment-mvp`**: replaced the anonymous, localStorage-only Assessment 2 app with a real login-based product. Google OAuth (`server/auth/google.js`) with `profile email gmail.readonly gmail.send calendar.events` scopes; SQLite persistence (`server/db.js`) for users, preferences, applicant details, applications, and conversation state — no more browser-only state.
  - **Switched reasoning layer from Gemini to Groq** (`server/services/groq.js`, model `openai/gpt-oss-120b`) — a deliberate move to escape Gemini's free-tier daily quota wall documented above, not a workaround; ported the same retry/backoff + never-fabricate-on-failure discipline.
  - **Real personalized event feed** (`server/routes/events.js`): real Tavily MCP search seeded from stored preferences, Groq-ranked, rendered on a dedicated Events page.
  - **Real browser-automated application skill** (`server/skills/formFiller.js`, Playwright): opens the actual Google Forms/Luma registration page in headless Chromium, reads real DOM fields, fills them from stored applicant details (never inventing structured facts), drafts short answers only for genuinely open-ended questions, and auto-submits only when every field was confidently resolved. Verified live against a real Luma event — real registration actually submitted, real Calendar event created, real Gmail confirmation email sent.
  - **"Fill a Form" review flow** (`server/routes/formReview.js`): paste any Google Forms/Luma link, agent fills it for review (screenshot + field list), human explicitly presses Approve & Submit — the human-approval gate the project requires, enforced by never re-resolving fields via Groq on submit (reuses the exact reviewed values).
  - **Paid-event handling**: real page-text scan for currency/payment language; paid events still get their form filled for review, but the agent never submits or touches payment — user completes manually, then can mark it "submitted" to start real Gmail-based approval tracking.
  - **Social-media-announcement handling**: when a search candidate is just a social post with no form, the agent now runs a real follow-up search for the actual registration link before giving up, instead of stopping at the post.
  - **Calendar blocking without a form**: even with no registration form to fill, the agent extracts a real, explicitly-stated date (never a post's publish date, never inferred from past-tense recap text — both confirmed via direct false-positive testing and prompt-fixed) and blocks the user's real Google Calendar, as an all-day event if only a date (no time) is known.
  - **Real Gmail approval/rejection tracking** (`server/cron/checkApplications.js`): daily + manual-trigger job reads real Gmail replies, classifies approved/rejected with Groq, updates the application and removes the calendar event on rejection.
  - **"Talk to Me" conversational agent** (`server/agent/talkToMeAgent.js`, new): a real multi-turn PERCEIVE→ACT→FILTER→CROSS-CHECK→SHORTLIST loop with server-side conversation memory (`chat_state` table) that survives navigation. Groq resolves references to earlier turns ("only weekend ones" correctly reused a prior "Pune"/"workshop" context without the user repeating it — verified live) and classifies intent (workshops/trends/discover/chat); a deterministic hard location filter (generalized from the old Pune-only filter) still gates results, not model judgment.
  - **New Discover page** (`server/routes/discover.js`): real MCP image-search for designers/projects, Groq-structured, same null-when-unstated discipline as Trends.
  - **Full UI/UX redesign** (`public/style.css` rewrite): editorial warm-palette design system; two-state homepage (minimal signed-out hero, signed-in "Talk to Me" hero + explore-card grid linking to Trends/Fill-a-Form/Discover/Events/Applications).
  - **Ranking quality fix**: Groq's event-ranking prompt now explicitly excludes personal testimonials/recap posts, after a real LinkedIn-style post ("I am happy to share my experience...") was surfaced as a fake event.
  - Applications page gained "I completed this manually" (moves a manually-finished application into real Gmail tracking) and "Remove" (deletes a bad record, cleaning up any real calendar event too).
- **What broke, and what I changed (all confirmed via real logs/tests, not assumed):**
  - **Google OAuth `redirect_uri_mismatch` / `invalid_request`**: traced through three real causes in sequence — a Railway env var containing a stale leftover query string instead of a clean URL, a conflicting project-level "shared variable" silently overriding the service-level one (confirmed by Railway's own "variable overwrite detected" prompt), and missing sensitive-scope declarations on the Google Cloud OAuth consent screen. Fixed all three; verified with a real cache-free Playwright login run showing the exact redirect_uri sent, not a guess.
  - **A real security leak**: the local SQLite database — containing real Google OAuth refresh tokens — had been committed to git on both branches (`data/` was never in `.gitignore`). Untracked it, fixed `.gitignore` on both branches, restored the real local data from a manual backup taken before the fix, and had the leaked token revoked via the user's own Google account permissions rather than attempting a risky git-history rewrite.
  - **Silent deploy failures**: Railway's production environment was connected to a stale branch (`hp-first-capston-project`) instead of `main` — every push to `main` had been silently not deploying. Found by comparing the live served HTML against what was actually on `origin/main`; fixed by repointing Railway's connected branch.
  - **A real fabricated-success bug I introduced and caught myself**: the generic (non-Google-Forms/Luma) application path was marking status `"applied"` and blocking the calendar even though nothing was ever actually submitted anywhere. Fixed to always report `needs_manual_action` with the detected fields shown only as reference — never a fabricated "applied."
  - **A real dead-code bug**: the daily/manual Gmail-approval check queried `status = 'pending'`, but successful applications are stored as `status = 'applied'` — meaning approval/rejection tracking had never actually run for any application, ever, until this was found and fixed.
  - **Calendar-date extraction false positive**: an early version of the date-extraction prompt misread a photo's "posted on" caption as the event's own date, and didn't recognize past-tense recap text as not being an upcoming event. Caught via a deliberate two-case test (a real recap post vs. a real future announcement) before shipping; fixed by tightening the prompt, then re-verified both cases correct.